...
<Callout type="info">
The default Docker setup starts Ogmios and uses it as Yaci Store's transaction evaluator. If Ogmios is unavailable, Yaci Store falls back to the embedded `scalus` evaluator. Kupo remains optional.
</Callout>

<Callout type="info">
Ogmios `v7.0.0` and Kupo `v2.12.0` are bundled as **native** binaries in both the `linux/amd64` and `linux/arm64` images — no x86_64 emulation is required on Apple Silicon or native Linux arm64 hosts.
</Callout>
...
#### Ogmios and Kupo
Ogmios is **enabled by default** (`ogmios_enabled=true` in `config/env`) and is used as Yaci Store's transaction evaluator. Set `ogmios_enabled=false` to disable it and fall back to the embedded `scalus` evaluator.

Kupo is optional and disabled by default. To activate it, either:

1. **Via env file**: Set `kupo_enabled=true` in `config/env`
2. **Via CLI**: Use the `enable-kupomios` command in Yaci CLI

When Yaci Store starts, DevKit uses the `ogmios` evaluator if Ogmios is enabled and responding on its health endpoint; otherwise it logs a warning and uses `scalus`.
