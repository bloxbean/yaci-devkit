<script lang="ts">
    ...
    // Define an interface for DRep De-registration data
    interface DRepDeRegistration {
        drep_id: string;
        cred_type: string;
        block_number: number;
        tx_hash: string;
    ...
    // --- Has More Logic --- 
    $: hasMore = data.deregistrations && data.deregistrations.length > 0;

    // --- Navigation Function --- 
    function goToPage(targetPage: number) {
        if (targetPage < 1 || loading) return;
        loading = true;
        const url = `/governance/drep-deregistrations?page=${targetPage}&count=${ITEMS_PER_PAGE}`;
        goto(url);
    }
    ...
    // Initialize deregistrations array if undefined
    if (!data.deregistrations) data.deregistrations = [];
</script>


<section class="container mx-auto text-sm">
    <h2 class="text-xl font-bold text-center text-base-content/60 mb-4">DRep De-registrations</h2>
    ...
    <div class="overflow-x-auto bg-base-100 shadow-md rounded-lg mb-4 relative">
        ...
        <table class="min-w-full divide-y divide-base-300">
            ...
            <!-- Iterate over drep de-registrations data -->
            {#each data.deregistrations as drep: DRepDeRegistration, index}
                <tr class="hover:bg-base-200">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-base-content">
                        <span class="ml-2">{drep.drep_id}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content/60">{drep.cred_type}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content/60"><a href="/blocks/{drep.block_number}" class="text-blue-500 hover:underline">{drep.block_number}</a></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content/60"><a href="/transactions/{drep.tx_hash}" class="text-blue-500 hover:underline">{drep.tx_hash}</a></td>
                </tr>
            {/each}
            </tbody>
        </table>
