<script lang="ts">
    ...
    // Define an interface for DRep Registration data
    interface DRepRegistration {
        drep_id: string;
        cred_type: string;
        block_number: number;
        tx_hash: string;
        anchor_url?: string; // Assuming these might be needed by DRepDetails
        anchor_hash?: string;
    }
    ...
    function goToPage(targetPage: number) {
        ...
        loading = true;
        const url = `/governance/drep-registrations?page=${targetPage}&count=${ITEMS_PER_PAGE}`;
        goto(url);
    }
    ...
    let selectedDRep: DRepRegistration | {} = {};

    const toggleDRep = (drep: DRepRegistration) => {
        selectedDRep = drep;
