...
<section class="container mx-auto text-sm">
    ...
    <h2 class="text-xl font-bold text-center text-base-content/60 mb-4">Stake Registrations</h2>
    <!-- ... existing buttons ... -->
    <div class="overflow-x-auto bg-base-100 shadow-md rounded-lg mb-4">
        <table class="min-w-full divide-y divide-base-300">
            ...
            {#each data.stakeRegistrations as registration: StakeRegistration, index}
                <tr class="hover:bg-base-200">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-base-content">
                        <div class="flex items-center gap-2">
                            <span>{truncate(registration.address, 20, "...")}</span>
                            <button 
                                class="text-base-content/50 hover:text-base-content/70" 
                                on:click={() => copyToClipboard(registration.address, 'Address copied to clipboard!')}
                                title="Copy address"
                            >
