...
<section class="bg-base-100 py-12">
    <div class="container mx-auto grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-base-200 rounded-lg p-4">
            ...
            <p class="text-base-content/80">{network_info.epoch}</p>
        </div>
        <div class="bg-base-200 rounded-lg p-4">
            <h3 class="text-xl font-semibold mb-2">Current Block</h3>
            <p class="text-base-content/80">{network_info.number}</p>
        </div>
        <div class="bg-base-200 rounded-lg p-4">
            <h3 class="text-xl font-semibold mb-2">Slot</h3>
            {#if network_info.epoch_slot}
                <p class="text-base-content/80">{network_info.epoch_slot}/{network_info.slots_per_epoch}</p>
            {/if}
        </div>
        <div class="bg-base-200 rounded-lg p-4">
            ...
            {#if network_info.epoch_progress}
                <div class="flex items-center">
                    <div class="w-3/4 bg-base-300 rounded-lg overflow-hidden">
                        <div class="bg-blue-500 h-2" style="width: {network_info.epoch_progress}%;"></div>
                    </div>
                    <p class="ml-2 text-base-content/80">{network_info.epoch_progress}%</p>
                </div>
            {/if}
        </div>
        <div class="bg-base-200 rounded-lg p-4">
            ...
            {#if network_info.total_active_stake}
                <div class="tooltip" data-tip={formatLovelace(parseInt(network_info.total_active_stake))}>
                    <p class="text-base-content/80">{formatAda(parseInt(network_info.total_active_stake))} ADA</p>
                </div>
            {:else}
                 <span>-</span>
            {/if}
        ...
        <div class="bg-base-200 rounded-lg p-4">
            ...
            {#if network_info.treasury}
                <div class="tooltip" data-tip={formatLovelace(parseInt(network_info.treasury))}>
                    <p class="text-base-content/80">{formatAda(parseInt(network_info.treasury))} ADA</p>
                </div>
            {:else}
                 <span>-</span>
            {/if}
        ...
        <div class="bg-base-200 rounded-lg p-4">
            ...
            {#if network_info.reserves}
                <div class="tooltip" data-tip={formatLovelace(parseInt(network_info.reserves))}>
                    <p class="text-base-content/80">{formatAda(parseInt(network_info.reserves))} ADA</p>
                </div>
            {:else}
                 <span>-</span>
            {/if}
        ...
        <div class="bg-base-200 rounded-lg p-4">
            ...
            {#if network_info.circulating_supply}
                <div class="tooltip" data-tip={formatLovelace(parseInt(network_info.circulating_supply))}>
                    <p class="text-base-content/80">{formatAda(parseInt(network_info.circulating_supply))} ADA</p>
                </div>
            {:else}
                <span>-</span>
            {/if}
