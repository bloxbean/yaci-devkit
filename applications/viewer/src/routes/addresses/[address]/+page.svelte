            ...
            <div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div class="p-6">
                    ...
                    <div class="text-gray-900 font-medium text-lg">
                        {lovelaceToAda(adaBalance, 2)} <span class="text-sm text-gray-500">ADA</span>
                    </div>
                </div>
        ...
        <div class="mb-6 bg-gray-50 rounded-xl p-2">
            <div class="flex flex-wrap gap-2 border-b border-gray-200">
                ...
                >
                    Tokens
                    {#if tokenCount > 0}
                        <span class="ml-1 bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs">{tokenCount}</span>
                    {/if}
                </button>
            </div>
        </div>
                                    ...
                                    {#each utxos as utxo (utxo.tx_hash + '#' + utxo.output_index)}
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-4 text-sm">
                                                ...
                                                {#if utxo.data_hash}
                                                    <div class="text-xs text-gray-400 mt-1" title={utxo.data_hash}>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    {#each tokens as token (token.unit)}
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 text-sm text-gray-700 font-mono" title={parsed.policyId}>
                                                <a href="/assets/policy/{parsed.policyId}" class="text-blue-600 hover:underline">
                                                    {truncate(parsed.policyId, 20, '...')}
                                                </a>
                                            </td>
                                            <td class="px-6 py-4 text-sm text-gray-700" title={parsed.assetNameHex}>
