...
<section class="py-8 px-4 md:px-6 min-h-screen">
    <div class="container mx-auto px-4 py-8">
        ...
        {#if showToast}
            <div class="fixed bottom-6 right-6 bg-neutral text-neutral-content px-5 py-3 rounded-lg shadow-xl flex items-center gap-2 z-50 animate-fade-up">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
            ...
            <form on:submit|preventDefault={handleSearch} class="w-full md:w-[400px] relative">
                <input
                    ...
                    class="input input-bordered w-full bg-base-100 text-base-content pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-shadow"
                    spellcheck="false"
                />
        ...
        {:else if !data.blocks || data.blocks.length === 0}
            <div class="bg-base-100 rounded-xl shadow-sm border border-base-300 p-12 text-center mt-4">
                <h3 class="text-lg font-medium text-base-content">No blocks found</h3>
                <p class="mt-1 text-sm text-base-content/60">Waiting for new blocks to be produced...</p>
            </div>
        {:else}
                        ...
                        <tbody class="divide-y divide-base-200">
                            {#each data.blocks as block (block.number)}
                                <tr class="hover:bg-base-200/50 transition-colors group">
                                    ...
                                    <td class="py-4 px-6 align-middle">
                                        <span class="text-sm text-base-content/70">{block.slot}</span>
                                    </td>
                                    <td class="py-4 px-6 align-middle">
                                        <div class="flex items-center gap-2">
                                            <span class="font-mono text-sm text-base-content/80" title={block.slot_leader}>
                                    ...
                                    <td class="py-4 px-6 align-middle">
                                        <span class="text-sm text-base-content/70">{(block.size / 1024).toFixed(2)}</span>
                                    </td>
                                    <td class="py-4 px-6 align-middle">
                    ...
                    {#each data.blocks as block (block.number)}
                        <div class="p-4 hover:bg-base-200/50 transition-colors">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    <div class="text-xs font-bold text-base-content/50 uppercase tracking-wider mb-1">Block</div>
                                    <a href="/blocks/{block.number}" class="font-mono text-sm text-blue-600 hover:underline font-medium">
                                        {block.number}
                                    </a>
                                </div>
                                <button class="p-2 bg-base-200 text-base-content/70 rounded-md active:bg-base-300 shrink-0" on:click={() => copyToClipboard(String(block.number))}>
                                    {@html iconCopy}
                                </button>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 mb-3">
                                <div class="bg-base-200 p-3 rounded-lg border border-base-200">
                                    <div class="text-xs text-base-content/60 mb-1">Slot</div>
                                    <div class="text-sm font-medium text-base-content">{block.slot}</div>
                                </div>
                                <div class="bg-base-200 p-3 rounded-lg border border-base-200">
                                    <div class="text-xs text-base-content/60 mb-1"># of Txs</div>
                                    <div class="text-sm font-medium text-base-content">{block.tx_count}</div>
                                </div>
                            </div>

                            <div class="flex justify-between items-center text-xs text-base-content/60 border-t border-base-200 pt-3">
                                <div class="truncate mr-4">
                                    Pool: <span class="font-mono font-medium text-base-content/80" title={block.slot_leader}>{truncate(block.slot_leader, 20)}</span>
                                </div>
                                <div class="shrink-0">
                                    Size: <span class="font-medium text-base-content/80">{(block.size / 1024).toFixed(2)} KB</span>
                                </div>
                            </div>
                        </div>
                    {/each}
